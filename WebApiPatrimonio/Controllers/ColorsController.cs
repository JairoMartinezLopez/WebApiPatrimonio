...
namespace WebApiPatrimonio.Controllers
{
    ...
    public class ColoresController : ControllerBase
    {
        ...
        [HttpGet("filtar")]
        public async Task<ActionResult<IEnumerable<Color>>> filtrarColores(
        ...
        public async Task<IActionResult> PutColores([FromBody] Color request)
        {
            ...
            command.CommandText = "PA_UPD_CAT_COLORES";

            command.Parameters.Add(new SqlParameter("@idColor", request.idColor));
            ...
            try
            {
                ...
                return Ok(new { mensaje = "El color a sido modificado correctamente." });
            }
        ...
        // To protect from overposting attacks, see https://go.microsoft.com/fwlink/?linkid=2123754
        [HttpPost]
        public async Task<ActionResult<Accion>> PostColor(Color request)
        {
            // Obtener el ID del usuario autenticado
            /*var userIdClaim = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier);
            if (userIdClaim == null || !int.TryParse(userIdClaim.Value, out int loggedInUserId))
            ...
            command.CommandText = "PA_INS_CAT_COLORES";
            ...
            try
            {
                await _context.Database.OpenConnectionAsync();
                await command.ExecuteNonQueryAsync();
                return Ok(new { mensaje = "Color insertado correctamente." });
            }
            catch (SqlException ex)
            {
                return BadRequest(new { error = ex.Message });
        ...
        public async Task<IActionResult> DeleteColor(int idColor)
        {
            ...
            var sql = "EXEC PA_DEL_CAT_COLORES @idColor, @IdPantalla, @IdGeneral";
            ...
            return Ok(new { mensaje = "Color eliminado lógicamente." });
        }
